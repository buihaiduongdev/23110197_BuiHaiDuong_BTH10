...
public class CategoryController {
    ...
    @PostMapping("/add")
    public String addCategory(@Valid @ModelAttribute("category") Category category, BindingResult bindingResult, @RequestParam("imageFile") MultipartFile imageFile, HttpSession session, Model model) {
        if (!isUserAuthorized(session)) {
            return "redirect:/login";
        }
    ...
    @PostMapping("/edit/{id}")
    public String editCategory(@PathVariable("id") Integer id, @Valid @ModelAttribute("category") Category category, BindingResult bindingResult, @RequestParam("imageFile") MultipartFile imageFile, HttpSession session, Model model) {
        if (!isUserAuthorized(session)) {
            return "redirect:/login";
        }
        ...
        User currentUser = (User) session.getAttribute("currentUser");
        Category existingCategory = categoryService.findById(id).orElseThrow(() -> new IllegalArgumentException("Invalid category Id:" + id));
        category.setCreatedBy(existingCategory.getCreatedBy()); // Preserve original creator
        category.setCreatedBy(currentUser); // Or update to current editor if you prefer
